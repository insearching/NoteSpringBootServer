...
class AuthService(
    ...
    fun refresh(refreshToken: String): TokenPair {
        ...
        val user = userRepository.findById(ObjectId(userId))
            .orElseThrow { throw ResponseStatusException(HttpStatusCode.valueOf(401), "Invalid refresh token.") }
        val hashed = hashToken(refreshToken)
        refreshTokenRepository.findByUserIdAndHashedToken(user.id, hashed)
            ?: throw ResponseStatusException(
                HttpStatusCode.valueOf(401),
                "Refresh token not recognized.(maybe ued or expired?)"
            )
    ...
    private fun storeRefreshToken(userId: ObjectId, refreshToken: String) {
        val hashed = hashEncoder.encode(refreshToken)
